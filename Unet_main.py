...
class BrainSegmentationDataset(Dataset):
    ...
    def __init__(
        ...
        # images_dir = "./lgg-mri-segmentation/kaggle_3m" 아래의 모든 폴더를 순회하여 읽음
        for (dirpath, dirnames, filenames) in os.walk(images_dir):
            # dirpath: 현재 탐색 중인 폴더의 전체 경로 (예: .../kaggle_3m/TCGA_CS_4941_19960909)
            # dirnames: 현재 폴더 안의 하위 폴더 리스트
            # filenames: 현재 폴더 안의 파일 리스트 (예: ..._1.tif, ..._1_mask.tif 등)
        
            image_slices = []
            mask_slices = []

            # 한 환자 폴더에서 이미지와 마스크 읽기
...
class UNet(nn.Module):
    ...
    def forward(self, x):
        enc1 = self.encoder1(x)                 # (Batch, features, H, W)
        enc2 = self.encoder2(self.pool1(enc1))  # (Batch, features*2, H/2, W/2)
        enc3 = self.encoder3(self.pool2(enc2))  # (Batch, features*4, H/4, W/4)
        enc4 = self.encoder4(self.pool3(enc3))  # (Batch, features*8, H/8, W/8)

        bottleneck = self.bottleneck(self.pool4(enc4)) # (Batch, features*16, H/16, W/16)

        dec4 = self.upconv4(bottleneck)         # (Batch, features*8, H/8, W/8)
        dec4 = torch.cat((dec4, enc4), dim=1)   # (Batch, features*16, H/8, W/8)
        dec4 = self.decoder4(dec4)              # (Batch, features*8, H/8, W/8)
        dec3 = self.upconv3(dec4)               # (Batch, features*4, H/4, W/4)
        dec3 = torch.cat((dec3, enc3), dim=1)   # (Batch, features*8, H/4, W/4)
        dec3 = self.decoder3(dec3)              # (Batch, features*4, H/4, W/4)
        dec2 = self.upconv2(dec3)               # (Batch, features*2, H/2, W/2)
        dec2 = torch.cat((dec2, enc2), dim=1)   # (Batch, features*4, H/2, W/2)
        dec2 = self.decoder2(dec2)              # (Batch, features*2, H/2, W/2)
        dec1 = self.upconv1(dec2)               # (Batch, features, H, W)
        dec1 = torch.cat((dec1, enc1), dim=1)   # (Batch, features*2, H, W)
        dec1 = self.decoder1(dec1)              # (Batch, features, H, W)
